...
DEVICE="intel_backlight"            # <- change if needed
STEP="5%"                           # percent step
...
# send and capture the notification id, then store it
# -p prints the id, -r tries replacement; using both keeps compatibility
NEW_ID=$($NOTIFY -p -t 1200 -r "$OLD_ID" -u low -h int:value:"$PERC" -i display-brightness-symbolic "Brightness" "${PERC}%")
# if notify-send returned nothing, fallback to a static number (still ok)
if [ -z "$NEW_ID" ]; then
  NEW_ID="$OLD_ID"
fi
echo "$NEW_ID" > "$ID_FILE"
